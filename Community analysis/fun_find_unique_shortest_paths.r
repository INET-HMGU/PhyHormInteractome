...
find_unique_shortest_paths <- function(my.graph,ahd,dbg){
  ahd <- ahd[,1:9]
  ...
  pdf(paste(result.dir,file.name," - Hormone Distance Boxplots UNIQUE PATHS.pdf",sep=""),width = 10, height = 10)
  ...
  pdf(paste(result.dir,file.name," - Hormone Distance Matrix UNIQUE PATHS.pdf",sep=""),width = 4, height = 4)
  ...
  write.xlsx(myMeanMatrix, paste(result.dir, file.name," - Shortest Paths - unique path.xlsx", sep = ""), sheetName = "Mean Values")
  write.xlsx(myMedianMatrix, paste(result.dir, file.name," - Shortest Paths - unique path.xlsx", sep = ""), sheetName = "Median Values", append=T)
