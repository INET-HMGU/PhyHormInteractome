...
find_unique_shortest_paths_wo_multiple <- function(my.graph,ahd,dbg,maxHormoneAnnotations){
  ahd <- ahd[,1:9]
  ...
  for(myHorm in 2:9){
    ...
    noAnno <- apply(ahd[,2:9],1,sum) # number of hormone annotations per protein / gene
    myIndex <- which(ahd[,myHorm] == 1 & as.numeric(noAnno) <= maxHormoneAnnotations ) # extension 2017/08/23: allow only nodes, which have not more than x annotations
      ...
      for(i in 1:(length(myNames))){
        ...
        if(i < length(myNames)){
          for(j in (i+1):length(myNames)){
            ...
            # extension: test, if the shortest path contains an annotated protein with the same hormone
            addPath <- T
            for(k in 1:length(myShortestPath$vpath)){
              nodesOnPath <- names(myShortestPath$vpath[[k]])[-c(1,length(myShortestPath$vpath[[k]]))]
              if(any(nodesOnPath %in% myNames)){
                addPath <- F
              }
            }
            
            if(addPath){
              # length of the first shortest path
              myLength <- length(myShortestPath$vpath[[1]])
              # add length of the shortest path to the list of shortest paths
        ...
        for(myNames2 in myNamesList){
          ...
          if(length(myNames2) > 0){
            for(j in 1:length(myNames2)){ # test length 
              ...
              # extension: test, if the shortest path contains an annotated proteins with the same hormone
              addPath <- T
              for(k in 1:length(myShortestPath$vpath)){
                nodesOnPath <- names(myShortestPath$vpath[[k]])[-c(1,length(myShortestPath$vpath[[k]]))]
                if(any(nodesOnPath %in% myNames2)){
                  addPath <- F
                }
              }
  ...
  pdf(paste(result.dir,file.name," - Hormone Distance Boxplots UNIQUE PATHS-max ",maxHormoneAnnotations,".pdf",sep=""),width = 10, height = 10)
  ...
  pdf(paste(result.dir,file.name," - Hormone Distance Matrix UNIQUE PATHS-max ",maxHormoneAnnotations,".pdf",sep=""),width = 4, height = 4)
  ...
  #                 col.regions=marker$color,colorkey = list(at = c(0:colorRange)), main=list('Mean Distance - unique path', side=1, line = 0.5))
  #       + layer(panel.text(x = x, y=y,  round(z,1), data=outMean)))
  ...
  #                 col.regions=marker$color,colorkey = list(at = c(0:colorRange)), main=list('Median Distance - unique path', side=1, line = 0.5))
  #       + layer(panel.text(x = x, y=y,  z, data=outMedian)))
  ...
  write.xlsx(
    ...
    paste(
      ...
      " - Shortest Paths - unique path wo multiple - max ",
      maxHormoneAnnotations,
      ".xlsx",
      sep = ""
    ),
    sheetName = "Mean Values",
    append = F
  )
